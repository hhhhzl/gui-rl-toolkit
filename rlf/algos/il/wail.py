...
class WailDiscrim(BaseIRLAlgo):
    ...
    def _compute_discrim_loss(self, agent_batch, expert_batch, obsfilt):
        ...
        diff = torch.cat([(expert_states - agent_states).flatten(1), (expert_actions - agent_actions).flatten(1)],
                         dim=-1)
    ...
    def _update_reward_func(self, storage):
        ...
        for epoch_i in range(self.args.n_wail_epochs):
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                n += 1
                expert_d, agent_d, grad_pen, regularize_term = self._compute_discrim_loss(agent_batch, expert_batch,
                                                                                          obsfilt)
